...
from ccnblplt import plotTopicResult
import shelve

parser = OptionParser()
parser.add_option('-d','--dataset',action='store',type='string',dest='dataset',metavar='dataset',default='Scene15',help='visual dataset')
...
def ccnblTopic():
    ...
    resultfile = '{0}{1}{2}{3}{4}{5}{6}'.format(rootDir,dataset,outDir,'ssrbcknnTopic',ccType,str(nTopic),str(nCodeword))
    ...
    result['blStd'] = blStd
    result.close()
    outPath = '{0}{1}{2}{3}{4}{5}{6}{7}{8}'.format(rootDir,dataset,outDir,'ssrbcTopic',ccType,str(nTopic),str(nCodeword),'.',figfmt)
    try:
        plotTopicResult(ccResult,blResult,catList,dataset,outPath,figfmt)
    except:
